...
def weak_text_prediction(text_prediction):
    if not text_prediction:
        return True
    return (
        text_prediction.get("source") == "semantic-fallback"
        and text_prediction.get("confidence", 0) < 0.48
        and not text_prediction.get("matchedKeywords")
    )


def weak_image_prediction(image_prediction, vision_result, vision_margin):
    if not image_prediction:
        return True
    return (
        bool((vision_result or {}).get("fallbackUsed", True))
        and image_prediction.get("confidence", 0) < 0.5
        and vision_margin < 0.1
    )
...
def build_structured_decision(
    ...
    return {
        ...
        "reasoning": {
            ...
            "abstentionReason": "Insufficient reliable text/image evidence." if insufficient_evidence else "",
        },
        "quality": {
